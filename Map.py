...
from CrateDebris import CrateDebris
...
class Map(object):
    ...
    def loadWorld_11(self):
        tmx_data = load_pygame("mario/Next/worlds/1-1/W11.tmx")
        ...
        # Crates - distributed across the level
        self.spawn_crate(800, 320)    # Near first goomba
        self.spawn_crate(1600, 320)   # Before first spike pit
        self.spawn_crate(2000, 288)   # On platform
        self.spawn_crate(3200, 320)   # In middle section
        self.spawn_crate(5500, 320)   # Near end section
    ...
    def spawn_crate(self, x, y):
        self.crates.append(Crate(x, y))

    def spawn_crate_text(self, x, y, crate_count):
        """
        This text appears when player collects a crate.
        """
        self.crate_debris.append(CrateDebris(x, y, crate_count))
    ...
    def update(self, core):
        ...
        for crate in self.crates:
            crate.update(core)
            if pg.Rect.colliderect(self.get_player().rect, crate.rect):
                crate.collect(core)

        # Crate collection text
        for crate_text in self.crate_debris:
            crate_text.update(core)
